...
class App_lib
{
    ...
    public function getStaffListTeacher( $role='')
    {
    ...
    public function getClass($branch_id = '')
    {
        if (empty($branch_id)) {
            ...
            $array = array('' => translate('select'));
            $array['all'] = translate('Todos los grados');
    ...
    public function getClassAdmision($branch_id = '') {
        if (empty($branch_id)) {
            $array = array('' => translate('select_branch_first'));
        } else {
            $getClassTeacher = $this->getClassTeacher();
            ...
            // Separar los elementos con 'Tarde' de los que no la tienen
            $resultWithTarde = [];
            $resultWithoutTarde = [];
    
            foreach ($result as $row) {
                if (strpos($row['name'], 'Tarde') !== false) {
                    $resultWithTarde[] = $row;
                } else {
                    $resultWithoutTarde[] = $row;
                }
            }
    
            // Ordenar los elementos con 'Tarde'
            usort($resultWithTarde, function($a, $b) {
                // Comparar solo si la parte común sin 'Tarde' es igual
                $commonNameA = str_replace('tarde', '', $a['name']);
                $commonNameB = str_replace('tarde', '', $b['name']);
    
                if ($commonNameA === $commonNameB) {
                    return strnatcasecmp($a['name'], $b['name']);
                }
    
                return strnatcasecmp($commonNameA, $commonNameB);
            });
    
            // Combinar los resultados ordenados
            $sortedResult = array_merge($resultWithoutTarde, $resultWithTarde);
    ...
    public function getSelectByBranch2($table, $branch_id = '', $all = false, $where = '')
    {
        if (empty($branch_id)) {
            $array = array('' => translate('select_branch_first'));
        } else {
            ...
            foreach ($result as $row) {
                $array[$row->id] = $row->name.' '.$row->number;
            }
        }
        return $array;
    }
    ...
    public function getSelectListStudent($table, $all = '')
    {
        $arrayData = array("" => translate('select'));
        if ($all == 'all') {
            $arrayData['all'] = translate('all_select');
        ...
        $result = $this->CI->db->get($table)->result();
        foreach ($result as $row) {
            $arrayData[$row->id] = $row->last_name." ".$row->first_name;
        }
        return $arrayData;
    }
