...
class Fees_model extends MY_Model
{
    ...
    public function getInvoiceDetails($studentID = '')
    {
        ...
        `fee_allocation`.`student_id` = " . $this->db->escape($studentID) . " AND `fee_allocation`.`session_id` = " . $this->db->escape(get_session_id()) . " ORDER BY `fee_allocation`.`group_id` ASC, `fee_groups_details`.`due_date` ASC , `fee_groups_details`.`fee_type_id` ";
        $student = array();
    ...
    {
        $sql = "SELECT IFNULL(SUM(h.amount), '0') as total_amount, d.discount as total_discount, gd.amount as full_amount, gd.due_date, e.student_id, e.roll, s.first_name, s.last_name,
        ...
        enroll as e ON e.student_id = fa.student_id LEFT JOIN student as s ON s.id = e.student_id LEFT JOIN class as c ON c.id = e.class_id 
        LEFT JOIN section as se ON se.id = e.section_id LEFT JOIN descuentos AS d ON d.allocation_id = fa.id AND d.type_id =  gd.fee_type_id
        WHERE
