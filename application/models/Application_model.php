...
class Application_model extends CI_Model
{
    ...
    //SE AGREGO EL CASO CUANDO ''CLASS ES ALL''
    public function getStudentListByClassSection($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false)
    {
        $sql = "SELECT `e`.*, `s`.`photo`, CONCAT_WS(' ',`s`.`first_name`, `s`.`last_name`) as `fullname`, `s`.`register_no`, `s`.`parent_id`, `s`.`email`, `s`.`mobileno`, `s`.`blood_group`,
         `s`.`birthday`, `s`.`admission_date`, `l`.`active`, `c`.`name` as `class_name`, `se`.`name` as `section_name`, `sc`.`name` as `category` FROM `enroll` as `e`
          INNER JOIN `student` as `s` ON `e`.`student_id` = `s`.`id` INNER JOIN `login_credential` as `l` ON `l`.`user_id` = `s`.`id` and `l`.`role` = 7 
          LEFT JOIN `class` as `c` ON `e`.`class_id` = `c`.`id` LEFT JOIN `section` as `se` ON `e`.`section_id`=`se`.`id` LEFT JOIN `student_category` as `sc` ON `sc`.`id` = `s`.`category_id`
           WHERE `e`.`branch_id` = " . $this->db->escape($branchID) . " AND `e`.`session_id` = " . $this->db->escape(get_session_id());
        
        if ($classID != 'all') {
            $sql .= " AND `e`.`class_id` = " . $this->db->escape($classID);
        }
    ...
    public function getStudentReservationListByClassSection($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false)
    {
        $sql = "SELECT 
        `e`.*, 
        `s`.`photo`, 
        CONCAT_WS(' ',`s`.`last_name`, `s`.`first_name`) as `fullname`, 
        `s`.`register_no`, 
        `s`.`parent_id`, 
        `s`.`email`, 
        `s`.`mobileno`, 
        ...
        `sc`.`name` as `category`, 
        IF(`r`.`student_id` IS NOT NULL, 1, 0) AS `RESERVA` 
    FROM `enroll` as `e` 
    ...
    LEFT JOIN `reservation` as `r` ON `r`.`student_id` = `e`.`student_id` AND `r`.`session_id` = " . $this->db->escape(get_session_id()+1) . "
    ...
    public function getStudentReservationListByClassSectionAntiguos($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false)
    {
        $sql = "SELECT 
        `e`.*, 
        `s`.`photo`, 
    ...
    LEFT JOIN `reservation` as `r` ON `r`.`student_id` = `e`.`student_id` AND `r`.`session_id` = " . $this->db->escape(get_session_id()+1) . "
    WHERE  `e`.`branch_id` = " . $this->db->escape($branchID);
    $sql .= " AND `s`.`created_at` < '2023-12-01'";
