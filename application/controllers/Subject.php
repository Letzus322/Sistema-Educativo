...
class Subject extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['subjectlist'] =  $this->db->get('subject')->result_array(); 
        $this->data['arealist'] =  $this->db->get('area')->result_array();
        ...
        if ($_POST) {
            if (get_permission('subject_class_assign', 'is_add')) {
                ...
                if ($this->form_validation->run() !== false) {
                    ...
                    $gradoMultisede= $this->input->post('class_id');

                    $this->db->select('sections_allocation.*,class.branch_id AS branch');
                    $this->db->from('grado');
                    $this->db->join('class', 'grado.id = class.grado_id');
                    $this->db->join('sections_allocation', 'class.id = sections_allocation.class_id');
                    $this->db->where('grado.id', $gradoMultisede);

                    $query = $this->db->get();
                    $multisedes = $query->result_array();

                    foreach ($multisedes as $multisede) {
                    $arraySubject = array(
                        'class_id' => $multisede['class_id'],
                        'section_id' => $multisede['section_id'],
                        'session_id' => get_session_id(),
                        'branch_id' => $multisede['branch'],
                    );
